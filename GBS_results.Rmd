...
```{r, fig.height=5, fig.width=8, cache=TRUE, echo=FALSE, fig.cap="Using MS as the two gro (Density plot of DF1). Note how the sexuals are spread out, but the apos are in dense spikes..."}
<<DAPC.ms>>
```
...
```{r, fig.height=5, fig.width=8, cache=TRUE, echo=FALSE, fig.cap="Pops as groups - colored by population from south (blue) to north (red). We can see that all of the Laramie sexuals (and some of the Laramie apos) all glom together..."}
<<DAPC.popall>>
```

<br/><br/>

```{r, fig.height=5, fig.width=8, cache=TRUE, echo=FALSE, fig.cap="Pops as groups - sub C59-S SM-A C23-A S03-A to `zoom in' a bit..."}
<<DAPC.popsub>>
```
...
I've calculated observed heterozygosity by ploidy level and by apo MLG. As expected, triploids and diploids have much higher levels of heterozygosity, with the mean Ho for polyploids being around double the diploids. Looking at the within MLG Ho (I tried to name them based on geography - MT_big is the big multi-pop MLG and Lrmy_big is the widespread Laramie clone.). This may be an indication of clone age, where we'd expect a negative relationship between heterozygosity and age (I think that's right?). 
