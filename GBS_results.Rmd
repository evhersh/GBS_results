...
I also calculated pairwise distances within populations and plotted them. This is similar to the IBS plot I made for my AGA poster, but using prevosti distance (raw dissimilarity) instead of IBS (% similarity). The R package I was using to calculate IBS (SNPRelate) was diploidizing my data without telling me, so I've switched to adegenet/poppr which can handle multiple ploidy levels. However, it's also worth noting that Meirmans et al (2018) report that this exact type of comparison (diploid-diploid vs polyploid-polyploid) is biased, so we should take these results with a grain of salt...
