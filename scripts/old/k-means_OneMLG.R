...
grp2 <- find.clusters(OneMLG.gc, max.n.clust=30) # looks like lowest BIC is 13 clusters...
names(grp2)
table(pop(OneMLG.gc), grp2$grp)
table.value(table(pop(OneMLG.gc), grp2$grp), col.lab=paste("inf", 1:4),
...
for(i in 1:nrow(myMat2)){
  grp2 <- find.clusters(OneMLG.gc, n.pca = 200, choose.n.clust = FALSE,  max.n.clust = maxK2)
  myMat2[i,] <- grp2$Kstat
}
...
p01 # 2-5 clusters kind of looks right?
...
for(i in 1:length(dapc00_l)){
  set.seed(10)
  grp20_l[[i]] <- find.clusters(OneMLG.gc, n.pca = 200, n.clust = my_k00[i])
  dapc00_l[[i]] <- dapc(OneMLG.gc, pop = grp20_l[[i]]$grp, n.pca = 8, n.da = my_k00[i])
...
for(i in 1:length(dapc01_l)){
  set.seed(10)
  grp2_l[[i]] <- find.clusters(OneMLG.gc, n.pca = 200, n.clust = my_k01[i])
  dapc01_l[[i]] <- dapc(OneMLG.gc, pop = grp2_l[[i]]$grp, n.pca = 8, n.da = my_k01[i])
...
my_df01$pop <- factor(my_df01$pop, levels=c("B53-S", "B60-S", "B42-S", "B46-S", "B49-S", "L62-S", "L62-A", "L05-S", "L08-S", "L10-S", "L11-S", "L12-S", "L13-S", "L16-A", "L17-A", "L39-A", "L41-A","L45-S", "C23-A", "C43-A", "S03-A", "SM-A", "C59-S"))
