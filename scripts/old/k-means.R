...
p3 <- ggplot(my_df, aes(x = Sample, y = Posterior, fill = Group)) +
  ...
  theme(legend.position = "none",
        ...
        strip.text.x.top = element_text(angle = 90)) +
  scale_fill_manual(values = colors)  # Apply greyscale

p3

# Second Plot (p4)
p4 <- ggplot(ind.Hobs, aes(x = ind, y = Hobs)) +
  geom_bar(stat = "identity", color = "white", fill = "grey50") +  # Ensure bars are grey
...
theme_adjustment <- theme(
  ...
  panel.spacing = unit(0, "lines")  # Eliminates extra space
)
p3 <- p3 + theme_adjustment
p4 <- p4 + theme_adjustment

p4 / p3 + plot_layout(heights = c(1, 1))
